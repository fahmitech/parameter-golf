...
def submission_code_bytes() -> int:
    return len(Path(__file__).resolve().read_bytes())
...
class Hyperparameters:
    ...
    lr_schedule_reference_step_ms = float(os.environ.get("LR_SCHEDULE_REFERENCE_STEP_MS", "0"))
    ...
    expert_mixer_enabled = bool(int(os.environ.get("EXPERT_MIXER_ENABLED", "1")))
...
def main() -> None:
    ...
    if can_run_online and args.ttt_enabled and online_best_agree_order in {"overlay_first", "both"}:
    ...
    if can_run_online:
        ...
        elif online_best_agree_order in {"ttt_first", "both"}:
            run_online_best_agree("post_ttt")
    if distributed:
