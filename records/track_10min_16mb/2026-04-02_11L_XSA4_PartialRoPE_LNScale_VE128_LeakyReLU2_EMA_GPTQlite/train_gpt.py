...
class ValueEmbedding(nn.Module):
    ...
    def forward(self, token_ids: Tensor) -> Tensor:
        ...
        return h * self.scale.to(dtype=h.dtype)
class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: int, activation: str = "relu2", leaky_slope: float = 0.5):
        super().__init__()
...
class Block(nn.Module):
    def __init__(
        ...
        self.mlp = MLP(dim, mlp_mult, activation=mlp_activation, leaky_slope=mlp_leaky_slope)
...
def main() -> None:
    ...
    log0(f"mlp_activation:{args.mlp_activation} mlp_leaky_slope:{args.mlp_leaky_slope}")
    log0(
