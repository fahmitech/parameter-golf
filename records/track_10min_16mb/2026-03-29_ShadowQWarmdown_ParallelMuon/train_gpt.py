...
class Hyperparameters:
    ...
    # Default the sp1024 trunk closer to the strongest public recipe:
    # wider BigramHash and XSA on all layers.
    ...
    shadow_q_enabled = bool(int(os.environ.get("SHADOW_Q_ENABLED", "1")))
    shadow_q_start_scale = float(os.environ.get("SHADOW_Q_START_SCALE", 0.30))
    shadow_q_snapshot_every = int(os.environ.get("SHADOW_Q_SNAPSHOT_EVERY", 50))
    shadow_q_export = bool(int(os.environ.get("SHADOW_Q_EXPORT", "1")))
    shadow_q_auto_select = bool(int(os.environ.get("SHADOW_Q_AUTO_SELECT", "1")))
...
def fake_quantize_int6_ste(t: Tensor, clip_range: int = 31) -> Tensor:
    if t.ndim < 2:
        return t
    t32 = t.float()
    row_clip = t32.abs().amax(dim=1).clamp_min(1.0 / clip_range)
    scale = row_clip / clip_range
    q = torch.clamp(torch.round(t32 / scale[:, None]), -clip_range, clip_range)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        ...
        # Match the repo baseline runtime path: use PyTorch SDPA with GQA instead
        # of depending on a separate FlashAttention-3 Python module at import time.
...
class GPT(nn.Module):
    ...
    def _bank_weight(self, t: Tensor) -> Tensor:
        return fake_quantize_int6_ste(t) if self.training and self.bank_qat_enabled else t
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            q_w = self._bank_weight(self.qo_bank[i])
            k_w = self._bank_weight(self.kv_bank[i])
            v_w = self._bank_weight(self.kv_bank[n + i])
            out_w = self._bank_weight(self.qo_bank[n + i])
            up_w = self._bank_weight(self.mlp_up_bank[i])
            down_w = self._bank_weight(self.mlp_down_bank[i])
        ...
        for i in range(self.num_decoder_layers):
            ...
            q_w = self._bank_weight(self.qo_bank[bi])
            k_w = self._bank_weight(self.kv_bank[bi])
            v_w = self._bank_weight(self.kv_bank[n + bi])
            out_w = self._bank_weight(self.qo_bank[n + bi])
            up_w = self._bank_weight(self.mlp_up_bank[bi])
            down_w = self._bank_weight(self.mlp_down_bank[bi])
...
def build_shadow_roundtrip_state_dict(state_dict: dict[str, Tensor], num_layers: int) -> dict[str, Tensor]:
    sd_cpu = {k: v.detach().cpu() for k, v in state_dict.items()}
    unbanked_sd = _unbank_state_dict(sd_cpu, num_layers)
    quant_result, quant_meta = mixed_quantize_int6(unbanked_sd, {"mlp", "attn"})
    deq_unbanked = dequantize_mixed_int6(quant_result, quant_meta, unbanked_sd)
    return _rebank_state_dict(deq_unbanked, num_layers, sd_cpu)
...
def main() -> None:
    ...
    log0(
        f"shadow_q:enabled:{args.shadow_q_enabled} start_scale:{args.shadow_q_start_scale:.3f} "
        f"snapshot_every:{args.shadow_q_snapshot_every} export:{args.shadow_q_export} "
        f"auto_select:{args.shadow_q_auto_select}"
    )
    ...
    ema_decay = 0.997
    shadow_state_sum: dict[str, Tensor] | None = None
    ...
    while True:
        ...
        if args.shadow_q_enabled and scale < args.shadow_q_start_scale and not base_model.bank_qat_enabled:
            base_model.bank_qat_enabled = True
            log0(f"shadow_q:enabled step:{step} scale:{scale:.4f}")
        zero_grad_all()
        ...
        if (
            base_model.bank_qat_enabled
            and args.shadow_q_snapshot_every > 0
            and step % args.shadow_q_snapshot_every == 0
        ):
            shadow_snapshot = build_shadow_roundtrip_state_dict(base_model.state_dict(), args.num_layers)
            if shadow_state_sum is None:
                shadow_state_sum = {name: t.detach().float().clone() for name, t in shadow_snapshot.items()}
            else:
                for name, t in shadow_snapshot.items():
                    shadow_state_sum[name].add_(t.detach().float())
            shadow_state_count += 1
            log0(f"shadow_q:snapshot step:{step} count:{shadow_state_count}")
        should_log_train = (
    ...
    ema_export_state = {name: t.detach().cpu().clone() for name, t in base_model.state_dict().items()}
    selected_export_state = "ema"
    if shadow_state_sum is not None and shadow_state_count > 0:
        shadow_avg_state = {
            name: (t / shadow_state_count).to(dtype=ema_export_state[name].dtype)
            for name, t in shadow_state_sum.items()
        }
        log0(f"shadow_q:averaging snapshots:{shadow_state_count}")
        base_model.load_state_dict(shadow_avg_state, strict=True)
        torch.cuda.synchronize()
        t_shadow = time.perf_counter()
        shadow_val_loss, shadow_val_bpb = eval_val(
            args, compiled_model, rank, world_size, device, grad_accum_steps,
            val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
        )
        torch.cuda.synchronize()
        log0(
            f"DIAGNOSTIC shadow_q val_loss:{shadow_val_loss:.4f} val_bpb:{shadow_val_bpb:.4f} "
            f"eval_time:{1000.0 * (time.perf_counter() - t_shadow):.0f}ms"
        )
        use_shadow_export = args.shadow_q_export and (
            not args.shadow_q_auto_select or shadow_val_bpb <= diag_val_bpb
        )
        if use_shadow_export:
            selected_export_state = "shadow_q"
            log0(f"shadow_q:selected export_state:{selected_export_state}")
        else:
            base_model.load_state_dict(ema_export_state, strict=True)
            reason = "disabled" if not args.shadow_q_export else "auto_select_kept_ema"
            log0(f"shadow_q:reverted export_state:ema reason:{reason}")
    else:
        log0("shadow_q:no_snapshots export_state:ema")
    full_state_dict = base_model.state_dict()
    log0(f"export_state:{selected_export_state}")
