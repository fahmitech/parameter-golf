...
def _default_ve_layers(num_layers: int) -> str:
    if num_layers <= 1:
        return "0"
    return f"{max(0, num_layers - 2)},{num_layers - 1}"


def _resolve_layer_indices(spec: str, num_layers: int) -> list[int]:
    spec = spec.strip()
    if spec.lower() in {"", "auto"}:
        spec = _default_ve_layers(num_layers)
    indices: list[int] = []
...
def _default_compile_fullgraph() -> str:
    # Fullgraph compile is fine for shallow recurrence, but deeper loop counts
    # can explode compile time by fully unrolling the recurrent trace.
    recurrent_loops = int(os.environ.get("RECURRENT_LOOPS", 3))
    return "1" if recurrent_loops <= 2 else "0"
...
class Hyperparameters:
    ...
    ve_layers = os.environ.get("VE_LAYERS", "auto")
    ...
    compile_fullgraph = bool(int(os.environ.get("COMPILE_FULLGRAPH", _default_compile_fullgraph())))
...
class LoopAdapter(nn.Module):
    def __init__(self, dim: int, num_loops: int, rank: int):
        super().__init__()
        if rank <= 0:
            raise ValueError(f"loop_adapter_rank must be positive, got {rank}")
    ...
    def forward(self, x: Tensor, loop_idx: int) -> Tensor:
        residual = self.up(F.silu(self.down(x)))
        loop_embed = self.iter_embed.weight[loop_idx].to(dtype=x.dtype)
        return x + self.gate.to(dtype=x.dtype) * (loop_embed + residual)
...
class GPT(nn.Module):
    def __init__(
        ...
        self.embed_proj_rev = CastedLinear(model_dim, self.embed_dim, bias=False) if (self.embed_dim != model_dim and tie_embeddings) else None
        ...
        self.skip_weights = nn.Parameter(
            torch.ones(recurrent_loops, self.num_skip_weights, model_dim, dtype=torch.float32)
        )
        gate_init = torch.ones(1, dtype=torch.float32) if recurrent_loops == 1 else torch.linspace(1.0, -1.5, recurrent_loops, dtype=torch.float32)
        self.loop_gates = nn.Parameter(gate_init)
        ...
        self.loop_adapters = nn.ModuleList(
            [LoopAdapter(model_dim, recurrent_loops, loop_adapter_rank) for _ in range(num_layers)]
        )
        if rope_dims > 0:
            head_dim = model_dim // num_heads
        ...
        self.ve_layer_indices = _resolve_layer_indices(ve_layers, num_layers) if ve_enabled else []
    ...
    def _init_weights(self) -> None:
        ...
        depth_scale = self.num_layers * self.recurrent_loops
        proj_scale = 1.0 / math.sqrt(2 * depth_scale)
        ...
        for i in range(self.num_layers):
            ...
            nn.init.zeros_(self.qo_bank.data[self.num_layers + i])      # Out (zero init)
    ...
    def _compute_logits(self, x: Tensor) -> Tensor:
        if self.tie_embeddings:
            ...
            logits_proj = F.linear(proj, self.tok_emb.weight)
        else:
    ...
    def _forward_hidden(self, input_ids: Tensor) -> Tensor:
        ...
        for loop_idx in range(self.recurrent_loops):
            ...
            for i in range(self.num_decoder_layers):
                ...
                x = self.loop_adapters[bi](x, loop_idx)
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        ...
        return self._compute_logits(self._forward_hidden(input_ids))
...
def eval_val_sliding(
    ...
    base_model.eval()
    compiled_logits = (
        torch.compile(base_model.forward_logits, dynamic=False, fullgraph=compile_fullgraph)
        if compile_logits else base_model.forward_logits
    )
    with torch.inference_mode():
...
def main() -> None:
    ...
    compiled_model = (
        ...
        if args.compile_model else base_model
    )
    ...
    scalar_params.extend(p for _, p in base_model.loop_adapters.named_parameters())
    ...
    if base_model.embed_proj is not None:
        tok_params.append({"params": [base_model.embed_proj.weight], "lr": token_lr, "base_lr": token_lr})
    if base_model.embed_proj_rev is not None:
        tok_params.append({"params": [base_model.embed_proj_rev.weight], "lr": token_lr, "base_lr": token_lr})
    if base_model.bigram is not None:
    ...
    log0(
        ...
        f"recurrent_loops:{args.recurrent_loops} effective_layers:{args.num_layers * args.recurrent_loops}"
    )
    ...
    log0(f"loop_adapters:rank:{args.loop_adapter_rank}")
    log0(f"ve_layers:{base_model.ve_layer_indices}")
    log0(f"compile:model:{args.compile_model} fullgraph:{args.compile_fullgraph}")
    log0(
        f"tie_embeddings:{args.tie_embeddings} embed_lr:{token_lr} "
        f"head_lr:{args.head_lr if base_model.lm_head is not None else 0.0} "
        f"matrix_lr:{args.matrix_lr} scalar_lr:{args.scalar_lr}"
    )
    log0(f"embed_factorization:embed_dim:{base_model.embed_dim} model_dim:{args.model_dim}")
    log0(
    ...
    eval_model = GPT(
        vocab_size=args.vocab_size, num_layers=args.num_layers, recurrent_loops=args.recurrent_loops,
        model_dim=args.model_dim, embed_dim=args.embed_dim,
    ...
    compiled_eval = (
        torch.compile(eval_model, dynamic=False, fullgraph=args.compile_fullgraph)
        if args.compile_model else eval_model
    )
    torch.cuda.synchronize()
    ...
    if args.eval_stride > 0 and args.eval_stride < sw_seq_len:
        ...
        sw_val_loss, sw_val_bpb = eval_val_sliding(
            ...
            compile_logits=args.compile_model,
