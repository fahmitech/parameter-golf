...
class Hyperparameters:
    ...
    mlp_activation = os.environ.get("MLP_ACTIVATION", "asqu")
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,asqu",
    ).split(",")
    if pattern
)
...
class MLP(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        elif self.activation == "asqu":
            if self.asqu is None:
                raise RuntimeError("ASQU parameters are missing")
            x = self.asqu(x)
            return self.proj(x)
        else:
...
def main() -> None:
    ...
    asqu_params = [block.mlp.asqu.beta for block in base_model.blocks if getattr(block.mlp, "asqu", None) is not None]
    asqu_param_ids = {id(p) for p in asqu_params}
    scalar_params = [
        p
        for name, p in block_named_params
        if (p.ndim < 2 or any(pattern in name for pattern in CONTROL_TENSOR_NAME_PATTERNS)) and id(p) not in asqu_param_ids
    ]
    ...
    if asqu_params:
        scalar_groups.append({"params": asqu_params, "lr": args.asqu_lr, "base_lr": args.asqu_lr})
